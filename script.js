...
// ===== hard bugfix: stage-clear monologues must never use suspect portrait/name =====
function renderPlayerMonologue(title, lines, doneText="계속", onDone=null){
  let i=0;
  function draw(){
    $("puzzleRoot").innerHTML = pTitle(title) + `
...
const stageMonologues = {
  deck: {
    title:"갑판 기록",
    lines:[
      "사진의 시간이 맞춰졌다. 최태오가 쓰러진 뒤, 이 배는 어디에도 닿지 못하고 있었다.",
      "라운지에서 시작된 소란, 객실로 이어진 이동, 그리고 끊긴 통신.",
      "이건 단순한 사고가 아니다. 최태오의 마지막 동선을 따라가야 한다."
    ]
  },
  lounge: {
    title:"라운지 기록",
  ...
  medical: {
    title:"의무실 기록",
    lines:[
      "약품 보관함의 빈자리. 반려된 보고서. 그리고 같은 이름으로 이어지는 기록.",
      "강하나는 이 약을 모르는 사람이 아니다.",
      "라운지에서 최태오가 쓰러진 일도, 우연으로 넘길 수 없다."
    ]
  },
  corridor: {
    title:"복도 기록",
    lines:[
      "발자국은 객실 문 앞에서 멈춘다.",
      "문손잡이에는 누군가가 열고 들어간 흔적이 남아 있다.",
      "이민정은 객실 근처에 가지 않았다고 했다. 그 말은 더 이상 그대로 믿을 수 없다."
    ]
  },
  cabin: {
    title:"객실 기록",
    lines:[
      "깨진 잔과 흐트러진 천. 이 방에서는 분명 무언가가 있었다.",
      "통화 기록과 반려된 진정서까지 이어진다.",
      "이민정은 우연히 이 방에 온 게 아니다. 최태오를 직접 마주할 이유가 있었다."
    ]
  },
  office: {
    title:"사무실 기록",
    lines:[
      "접근 로그에 황세준의 이름이 남아 있다.",
      "통신실은 그의 권한 밖이 아니었다.",
      "보고가 멈춘 시간과도 맞아떨어진다. 그는 모른 척할 수 없다."
    ]
  }
};

function showDialogue(key){
  const m = stageMonologues[key];
  if(m){
    renderPlayerMonologue(m.title, m.lines);
    return;
  }
  closePuzzle();
}
