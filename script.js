 ...
 const rounds=[
  {
   ...
   profile:"kang",
   x:22,y:73,
   statement:"“최태오 대표와 제 가족은 아무 관계가 없어요. 저는 그냥 초대받은 손님이었을 뿐입니다.”",
   lie:"제 가족은 아무 관계가 없어요.",
   evidence:"가족 편지",
  ...
  },
  {
   ...
   profile:"lee",
   x:50,y:73,
   statement:"“저는 객실 안에 들어가지 않았어요. 그 사람과는 오늘 밤 처음 마주쳤습니다.”",
   lie:"객실 안에 들어가지 않았어요.",
   evidence:"문손잡이 흔적",
   evidenceId:"handle",
  ...
  },
  {
   ...
   profile:"hwang",
   x:78,y:73,
   statement:"“통신실은 제 권한 밖입니다. 보고가 멈춘 이유도 저는 모릅니다.”",
   lie:"통신실은 제 권한 밖입니다.",
   evidence:"접근 로그",
 ...
 function draw(e=""){
  ...
  $("puzzleRoot").innerHTML=pTitle("용의자 소집")+
   `<div class="summonScene">
    ...
    <div class="dialogue">
      <b>${r.speaker}</b><br><br>${r.statement}<br><br>
      <span style="color:var(--muted)">대화 속 모순되는 말과 제시할 단서를 고른다.</span>
    </div>
  ...
  [r.lie,"그날 라운지에는 가지 않았어요.","최태오 대표가 쓰러진 뒤에야 알았습니다."].forEach(x=>option("lieBtns",x,false,()=>{pickedLie=x;$("selectedLie").textContent=x}));
  [r.evidence,"갑판 사진 묶음","라운지 잔 기록"].forEach(x=>option("evidenceBtns",x,false,()=>{pickedEvidence=x;$("selectedEvidence").textContent=x}));

  document.querySelector(".nameTag.kang").onclick=()=>openProfile("kang");
  document.querySelector(".nameTag.lee").onclick=()=>openProfile("lee");
  document.querySelector(".nameTag.hwang").onclick=()=>openProfile("hwang");
  ...
  $("checkTruth").onclick=()=>{
   if(pickedLie===r.lie&&pickedEvidence===r.evidence){
    ...
    showResult(r);
   }else draw("모순 문장 또는 제시 단서가 맞지 않는다")
  };
 }
