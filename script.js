...
function openTruth(){
 ...
 function showResult(r){
  ...
  $("puzzleRoot").innerHTML=pTitle(`${r.speaker}의 거짓말`)+
   `<div class="summonScene">
      <img src="${A("scene-suspect-gathering.png")}" alt="">
      <button class="nameTag kang">강하나</button>
      <button class="nameTag lee">이민정</button>
...
function openTruth(){
  ...
  function showResult(){
    const r=rounds[step];
    note(`${r.speaker} / 모순 확인`);
    $("puzzleRoot").innerHTML=pTitle(`${r.speaker}의 거짓말`)+`<div class="dialogueLayout"><div class="vnPortraitWrap"><img class="vnPortrait" src="${portraitSrc(r.profile,'neutral')}" alt="${r.speaker}"></div><div class="vnBox"><div class="vnName">${r.speaker}</div><div class="dialogue">${r.result}</div><div class="row">${card(r.evidenceId,r.evidence)}</div><div class="actions"><span></span><button id="nextTruth" class="btn primary">${step===rounds.length-1?'통신실로':'다음 증언'}</button></div></div></div>`;
